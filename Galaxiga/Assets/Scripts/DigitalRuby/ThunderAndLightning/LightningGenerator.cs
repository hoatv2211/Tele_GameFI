	...
	public class LightningGenerator
	{
		...
		public void RandomVector(LightningBolt bolt, ref Vector3 start, ref Vector3 end, float offsetAmount, System.Random random, out Vector3 result)
		{
			if (bolt.CameraMode == CameraMode.Perspective)
			{
				...
				Vector3 vector = Vector3.Cross(start, end);
				if (vector == Vector3.zero)
				{
					this.GetPerpendicularVector(ref a, out vector);
				}
				else
				{
					vector.Normalize();
				...
				a *= (float)Math.Sin((double)num);
				Quaternion rotation;
				rotation.x = a.x;
				rotation.y = a.y;
				rotation.z = a.z;
				rotation.w = (float)Math.Cos((double)num);
				result = rotation * vector * d;
			}
			else if (bolt.CameraMode == CameraMode.OrthographicXY)
			{
				end.z = start.z;
				Vector3 normalized = (end - start).normalized;
				Vector3 a2 = new Vector3(-normalized.y, normalized.x, 0f);
			...
			else
			{
				...
				Vector3 a3 = new Vector3(-normalized2.z, 0f, normalized2.x);
		...
		public void GenerateLightningBolt(LightningBolt bolt, LightningBoltParameters p)
		{
			Vector3 vector;
			Vector3 vector2;
			this.GenerateLightningBolt(bolt, p, out vector, out vector2);
		}

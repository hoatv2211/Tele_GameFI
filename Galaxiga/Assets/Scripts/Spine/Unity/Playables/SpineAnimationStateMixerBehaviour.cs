...
namespace Spine.Unity.Playables
{
	public class SpineAnimationStateMixerBehaviour : PlayableBehaviour
	{
		public override void ProcessFrame(Playable playable, FrameData info, object playerData)
		{
			...
			{
				this.PreviewEditModePose(playable, skeletonAnimation);
				return;
			}
			...
			float[] array = this.lastInputWeights;
		...
		public void PreviewEditModePose(Playable playable, SkeletonAnimation spineComponent)
		{
			...
			if (num != -1)
			{
				...
				bool flag = behaviour.animationReference != null && spineComponent.SkeletonDataAsset.GetSkeletonData(true) != behaviour.animationReference.SkeletonDataAsset.GetSkeletonData(true);
				if (flag)
				{
					UnityEngine.Debug.LogWarningFormat("SpineAnimationStateMixerBehaviour tried to apply an animation for the wrong skeleton. Expected {0}. Was {1}", new object[]
					{
						spineComponent.SkeletonDataAsset,
						behaviour.animationReference.SkeletonDataAsset
					});
				}
				...
				float num2 = (float)playable2.GetTime<ScriptPlayable<SpineAnimationStateBehaviour>>();

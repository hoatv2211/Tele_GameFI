...
public class EventDispatcher : MonoBehaviour
{
	...
	{
		foreach (KeyValuePair<EventID, List<Action<Component, object>>> keyValuePair in this._listenersDict)
		{
			List<Action<Component, object>> value = keyValuePair.Value;
			...
			for (int i = count - 1; i >= 0; i--)
			{
				...
				if (action == null || action.Target.Equals(null))
				{
					...
					if (value.Count == 0)
					{
						this._listenersDict.Remove(keyValuePair.Key);
					}
					i--;
				}
			}
